...
#define BlockSize 16
#define PixelsPerThreads 10

__global__ void mandelKernel(float lowerX, float lowerY, float stepX, float stepY, int* img, int resX, int maxIterations, int pitch, int pixels) {

    
    for(int p = 0; p < pixels; ++p){
        int x = (blockIdx.x * blockDim.x + threadIdx.x) * pixels + p;
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    dim3 numBlocks(resX / (threadsPerBlock.x * PixelsPerThreads), resY / threadsPerBlock.y);
    mandelKernel<<<numBlocks, threadsPerBlock>>>(lowerX, lowerY, stepX, stepY, d_img, resX, maxIterations, pitch, PixelsPerThreads);
